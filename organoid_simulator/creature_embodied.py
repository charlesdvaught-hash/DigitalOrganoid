...
POOLS = {
    ...
    'cpg': (190, 194),  # Innate 4-neuron CPG oscillator pool
}
...
def build_creature_network(N, K, fi=0.2, seed=None, bilateral_symmetric=False):
    ...
    if bilateral_symmetric:
        # Enforce exact sagittal-plane mirror symmetry (x -> -x) between paired pools
    ...
    if bilateral_symmetric and N >= 2 * (N // 2):
        half = N // 2
        types[half:2*half] = types[:half]  # Symmetric excitatory/inhibitory types
...
def run_creature_simulation(net, T, dale=True, stdp_on=True, seed=None, record_every=1,
                           cpg_enabled=False, cerebellar_enabled=False,
                           multichannel_neuromod=False):
    ...
    # Cerebellar forward model parameters
    W1_cer = rng.standard_normal((8, 6)) * 0.1
    b1_cer = np.zeros(8)
    W2_cer = rng.standard_normal((4, 8)) * 0.1
    b2_cer = np.zeros(4)
    prev_s_vec = None
    prev_h_cer = None
    prev_s_pred = None
    ...
    for t in range(T):
        ...
        if alive:
            ...
            # Cerebellar Forward Model prediction update
            if cerebellar_enabled:
                s_actual = np.array([L, R, W, food_close])
                if prev_s_pred is not None:
                    e_cer = s_actual - prev_s_pred
                    pe_mag = float(np.linalg.norm(e_cer))
                    dW2 = np.outer(e_cer, prev_h_cer)
                    dh = (W2_cer.T @ e_cer) * (1.0 - prev_h_cer ** 2)
                    dW1 = np.outer(dh, prev_s_vec)
                    W2_cer += 0.01 * dW2
                    W1_cer += 0.01 * dW1

            # Multi-channel neuromodulation signals
            if multichannel_neuromod:
                serotonin = energy_val / 100.0
                norepinephrine = 0.95 * norepinephrine + 0.05 * pe_mag
                acetylcholine = max(L, R, food_close)
                current_sensor_factor = sensor_factor * (0.7 + 0.6 * acetylcholine)
                current_noise_scale = noise_scale * (1.0 + 0.5 * norepinephrine)
            else:
                current_sensor_factor = sensor_factor
                current_noise_scale = noise_scale

            sensor_inj[0:20] = L * current_sensor_factor
            sensor_inj[20:40] = R * current_sensor_factor
            sensor_inj[40:60] = W * current_sensor_factor
            sensor_inj[60:80] = food_close * current_sensor_factor
            sensor_inj[80:100] = hunger * current_sensor_factor
            sensor_inj[100:120] = tired * current_sensor_factor

            # Innate CPG oscillator drive
            if cpg_enabled:
                cpg_phase = 2.0 * np.pi * (t % 40) / 40.0
                cpg_l = 12.0 * (1.0 + np.sin(cpg_phase))
                cpg_r = 12.0 * (1.0 + np.sin(cpg_phase + np.pi))
                sensor_inj[mL0:mL1] += cpg_l
                sensor_inj[mR0:mR1] += cpg_r
        ...
        I_noise = rng.standard_normal(N) * current_noise_scale if alive else rng.standard_normal(N) * noise_scale
        ...
        if alive:
            ...
            # Store states for cerebellar predictor
            if cerebellar_enabled:
                s_vec = np.array([L, R, W, food_close, mL, mR])
                h_cer = np.tanh(W1_cer @ s_vec + b1_cer)
                prev_s_pred = W2_cer @ h_cer + b2_cer
                prev_s_vec = s_vec
                prev_h_cer = h_cer
            ...
            turn = (mL - mR) * turn_gain * MOTOR_GAIN + dh_wall * W * 0.3
