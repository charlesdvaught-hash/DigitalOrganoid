...
# Neuroforge-style pure-Hebbian learning mode constants.
HEBB_RATE = 0.02
HEBB_DECAY = 0.002
...
class BatchSim:
    ...
    def run(self, weight_plastic, T, reflex_scale, stdp_on=True, record_steering=False,
            ...
            prune_thresh=0.0, prune_every=500):
        ...
        for t in range(T):
            ...
            sensor_inj[:, 0:20] = (L * self.sensor_factor).unsqueeze(1) * av
            sensor_inj[:, 20:40] = (R * self.sensor_factor).unsqueeze(1) * av
            sensor_inj[:, 40:60] = (wall * self.sensor_factor).unsqueeze(1) * av
            sensor_inj[:, 60:80] = (food_close * self.sensor_factor).unsqueeze(1) * av
            if use_eye:
                sensor_inj[:, self.eye_lo:self.eye_hi] = eye_sig * self.sensor_factor * av
            ...
            if stdp_on and learning == 'hebbian':
                # Neuroforge-style: pure coincidence learning, no dopamine
                # gate, no eligibility trace, no reward tie-in at all --
                # "neurons that fire together wire together," full stop.
                # Includes constant decay (Neuroforge decays strength every
                # frame) so unused synapses fade instead of staying at
                # whatever evolution seeded them with.
                pf = fired[:, self.post_p]
                qf = fired[:, self.pre_p]
                co_fire = trace_pre[:, self.pre_p] * pf.to(dt)
                wp = wp * (1.0 - HEBB_DECAY) + HEBB_RATE * co_fire
                wp = torch.where(self.exc_mask.unsqueeze(0),
                                 torch.clamp(wp, 0.0, W_MAX),
                                 torch.clamp(wp, -W_MAX, 0.0))
                if prune_thresh > 0 and (t + 1) % max(1, prune_every) == 0:
                    # Neuroforge prunes idle/weak connections outright.
                    # Can't spawn new edges in this fixed shared-topology
                    # batched kernel, so this is prune-only, no regrowth --
                    # note that asymmetry when comparing results.
                    wp = torch.where(wp.abs() < prune_thresh, torch.zeros_like(wp), wp)

            if stdp_on and not surprise and learning != 'hebbian':
...
def run_lifetime(sim, weight_plastic, args, reflex_scale, T, record_steering=False):
    ...
    if args.bootcamp:
        boot_out = sim.run(wp, args.boot_t, reflex_scale, stdp_on=True,
                           ...
                           use_eye=args.use_eye,
                           prune_thresh=args.prune_thresh, prune_every=args.prune_every)
...
def main():
    ...
    ap.add_argument('--learning', type=str, default='stdp', choices=['stdp', 'surprise', 'hebbian'],
                    help='stdp = reward-gated instantaneous STDP; surprise = eligibility x TD-error; '
                         'hebbian = Neuroforge-style pure co-firing correlation, no reward tie-in at all')
    ...
    ap.add_argument('--prune-thresh', type=float, default=0.0,
                    help='hebbian mode only: zero out |weight|<thresh every --prune-every steps '
                         '(Neuroforge-style idle-connection pruning; no regrowth in this harness)')
    ap.add_argument('--prune-every', type=int, default=500)
